...
class TransformerEncoderLayer_cau(nn.Module):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        attn_mask_in = nn.Transformer.generate_square_subsequent_mask(src.shape[0])

        src2 = self.self_attn(src_norm, src_norm, src_norm, attn_mask=attn_mask_in,
...
class AIA_Transformer_cau(nn.Module):
    ...
    def __init__(self, input_size,output_size, dropout=0, num_layers=1):
        super(AIA_Transformer_cau, self).__init__()
        ...
        for i in range(num_layers):
            self.row_trans.append(TransformerEncoderLayer_cau(d_model=input_size//2, nhead=4, dropout=dropout, bidirectional=True))
            self.col_trans.append(TransformerEncoderLayer_cau(d_model=input_size//2, nhead=4, dropout=dropout, bidirectional=False))
...
class AIA_Transformer_new(nn.Module):
    ...
    def __init__(self, input_size,output_size, dropout=0, num_layers=1):
        ...
        self.col_norm = nn.ModuleList([])
        for i in range(num_layers):
            self.row_trans.append(TransformerEncoderLayer_new(d_model=input_size, nhead=4, dropout=dropout, bidirectional=True))
            self.col_trans.append(TransformerEncoderLayer_new(d_model=input_size, nhead=4, dropout=dropout, bidirectional=True))
...
class AIA_DCN_Transformer_merge(nn.Module):
    ...
    def forward(self, input1, input2):
        ...
        for i in range(len(self.row_trans)):
            ...
            AFA_input_mag = intra_out_DCN_i.permute(3, 0, 2, 1).contiguous().view(dim1, b*dim2, -1)  # [F, B*T, c]
            AFA_output_mag = self.row_trans[i](AFA_input_mag)  # [F, B*T, c]
            AFA_output_mag = AFA_output_mag.view(dim1, b, dim2, -1).permute(1, 3, 2, 0).contiguous()  # [B, C, T, F]
            AFA_output_mag = self.row_norm[i](AFA_output_mag)  # [B, C, T, F]

            ATA_input_mag = intra_out_DCN_i.permute(2, 0, 3, 1).contiguous().view(dim2, b*dim1, -1)  # [T, B*F, C]
            ATA_output_mag = self.col_trans[i](ATA_input_mag)  # [T, B*F, C]
            ATA_output_mag = ATA_output_mag.view(dim2, b, dim1, -1).permute(1, 3, 0, 2).contiguous()  # [B, C, T, F]
            ATA_output_mag = self.col_norm[i](ATA_output_mag)  # [B, C, T, F]
            ATA_output_temp_i = input_mag + self.k1*AFA_output_mag + self.k2*ATA_output_mag  # [B, C, T, F]
            ...
            #input_ri_mag = output_ri_i + output_list_mag[-1]
            AFA_input_ri = intra_out_DCN_ri.permute(3, 0, 2, 1).contiguous().view(dim1, b*dim2, -1)  # [F, B*T, c]
            AFA_output_ri = self.row_trans[i](AFA_input_ri)  # [F, B*T, c]
            AFA_output_ri = AFA_output_ri.view(dim1, b, dim2, -1).permute(1, 3, 2, 0).contiguous()  # [B, C, T, F]
            AFA_output_ri = self.row_norm[i](AFA_output_ri)  # [B, C, T, F]

            ATA_input_ri = intra_out_DCN_ri.permute(2, 0, 3, 1).contiguous().view(dim2, b*dim1, -1)  # [T, B*F, C]
            ATA_output_ri = self.col_trans[i](ATA_input_ri)  # [T, B*F, C]
            ATA_output_ri = ATA_output_ri.view(dim2, b, dim1, -1).permute(1, 3, 0, 2).contiguous()  # [B, C, T, F]
            ATA_output_ri = self.col_norm[i](ATA_output_ri)  # [B, C, T, F]
            ATA_output_temp_ri = input_ri + self.k1*AFA_output_ri + self.k2*ATA_output_ri # [B, C, T, F]

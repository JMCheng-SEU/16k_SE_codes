...
def magnitude_at_standard_listening_level(spectra):
    ...
    mask = np.zeros(int(Nf / 2 + 1), dtype=np.float32)
    ...
    Pow = torch.mean(torch.mul(spectra, torch.tensor(mask, dtype=torch.float32).to('cuda')))
    return spectra * (10000000.0 / (Pow+EPSILON))
...
def bark_computation(spectra):
    ...
    """
    bark_spectra = torch.matmul(spectra, bark_matrix) * Sp
    return bark_spectra
...
def compute_audible_power(bark_spectra, factor=1.0):
    ...
    """
    cond = torch.gt(bark_spectra, abs_thresh_power * factor)

    return torch.sum(torch.where(cond, bark_spectra, torch.zeros_like(bark_spectra)).to('cuda'), dim=1)
...
def bark_gain_equalization(bark_spectra_ref, bark_spectra_deg):
    ...
    gain = torch.div(audible_power_ref + 5.0e3, audible_power_deg + 5.0e3)

    minpad = (torch.zeros_like(gain)+5).to('cuda')
    equalizer = torch.where(gain > 5, minpad, gain)
    maxpad = (torch.zeros_like(equalizer)+3.0e-4).to('cuda')
    limited_gain = torch.where(gain <3.0e-4, maxpad, gain)

    return torch.mul(bark_spectra_deg, torch.unsqueeze(limited_gain, -1))
...
def bark_frequency_equalization(bark_spectra_ref, bark_spectra_deg):
    ...
    not_silent = torch.ge(audible_powerX100, 1.0e7)

    # Threshold for active bark bins (if value is lower than 100 times audible threshold then it is set to 0)
    ref_thresholded = torch.where(torch.ge(bark_spectra_ref, abs_thresh_power * 100.0), bark_spectra_ref,
                               torch.zeros_like(bark_spectra_ref))
    deg_thresholded = torch.where(torch.ge(bark_spectra_deg, abs_thresh_power * 100.0), bark_spectra_deg,
                               torch.zeros_like(bark_spectra_deg))

    # Average Pow per bark bin in reference and degraded signals
    avg_pow_per_bark_ref = torch.sum(torch.where(not_silent, ref_thresholded, tf.zeros(tf.shape(bark_spectra_ref))),
                                         dim=0)
    avg_pow_per_bark_deg = torch.sum(torch.where(not_silent, deg_thresholded, tf.zeros(tf.shape(bark_spectra_deg))),
                                         dim=0)

    equalizer = (avg_pow_per_bark_deg + 1000.0) / (avg_pow_per_bark_ref + 1000.0)
    minpad = torch.zeros_like(equalizer)+100
    equalizer = torch.where(equalizer > 100, minpad, equalizer)
    maxpad = torch.zeros_like(equalizer)+0.01
    equalizer = torch.where(equalizer <0.01, maxpad, equalizer)

    return torch.mul(bark_spectra_ref, equalizer)
...
def bark_frequency_equalization_on_degraded(bark_spectra_ref, bark_spectra_deg):
    ...
    # Identification of speech active frames (over reference bark spectra):
    audible_powerX100 = compute_audible_power(bark_spectra_ref, 100.0)
    not_silent = torch.ge(audible_powerX100, 1.0e7).unsqueeze(1)


    # Threshold for active bark bins (if value is lower than 100 times audible threshold then it is set to 0)
    ref_thresholded = torch.where(torch.ge(bark_spectra_ref, abs_thresh_power * 100.0), bark_spectra_ref,
                               torch.zeros_like(bark_spectra_ref).to('cuda'))
    deg_thresholded = torch.where(torch.ge(bark_spectra_ref, abs_thresh_power * 100.0), bark_spectra_deg,
                               torch.zeros_like(bark_spectra_deg).to('cuda'))

    # Average Pow per bark bin in reference and degraded signals
    # not_silent = not_silent.expand([not_silent.shape[0], 49])
    avg_pow_per_bark_ref = torch.sum(torch.where(not_silent, ref_thresholded, torch.zeros_like(bark_spectra_ref).to('cuda')),
                                         dim=0)
    avg_pow_per_bark_deg = torch.sum(torch.where(not_silent, deg_thresholded, torch.zeros_like(bark_spectra_deg).to('cuda')),
                                         dim=0)

    equalizer = (avg_pow_per_bark_ref + 1000.0) / (avg_pow_per_bark_deg + 1000.0)
    minpad = (torch.zeros_like(equalizer)+100).to('cuda')
    equalizer = torch.where(equalizer > 100, minpad, equalizer)
    maxpad = (torch.zeros_like(equalizer)+0.01).to('cuda')
    equalizer = torch.where(equalizer <0.01, maxpad, equalizer)

    return torch.mul(bark_spectra_deg, equalizer)
...
def loudness_computation(bark_spectra):
    ...
    """
    loudness_dens = Sl * torch.mul(torch.pow(abs_thresh_power / 0.5 + EPSILON, modified_zwicker_power),
                                     torch.pow(0.5 + 0.5 * bark_spectra / abs_thresh_power+ EPSILON, modified_zwicker_power) - 1.0)

    cond = torch.lt(bark_spectra, abs_thresh_power)
    loudness_dens_limited = torch.where(cond, torch.zeros_like(loudness_dens).to('cuda'), loudness_dens)
...
def compute_distortion_tensors(bark_spectra_ref, bark_spectra_deg):
    ...
    # Loudness difference
    r = torch.sub(distorted_loudness, original_loudness)
    # Masking effect computation
    m = 0.25 * torch.min(original_loudness, distorted_loudness)
    # Center clipping using masking effect

    temp = torch.abs(r) - m
    maxpad = torch.zeros_like(temp).to('cuda')
    D = torch.where(temp < 0, maxpad, temp)

    # Asymmetry factor computation
    Asym = torch.pow(torch.div(bark_spectra_deg + 50.0, bark_spectra_ref + 50.0)+ EPSILON, 1.2)
    cond = torch.lt(Asym, 3.0)

    maxpad = (torch.zeros_like(Asym) + 12).to('cuda')
    limited_Asym = torch.where(temp >12, maxpad, Asym)
    AF = torch.where(cond, torch.zeros_like(Asym).to('cuda'), limited_Asym)
...
def per_frame_distortion(D, DA, total_power_ref):
    ...
    D_frame = torch.sqrt(torch.sum((torch.mul(D, width_of_band_bark))**2, dim=1)+ EPSILON) * sqrt_total_width
    DA_frame = torch.sum(torch.mul(DA, width_of_band_bark), dim=1)
    ...
    minpad = (torch.zeros_like(torch.div(D_frame, weights))+ 45).to('cuda')
    wD_frame = torch.where(torch.div(D_frame, weights) >45, minpad, torch.div(D_frame, weights+ EPSILON))

    minpad = (torch.zeros_like(torch.div(DA_frame, weights))+ 45).to('cuda')
    wDA_frame = torch.where(torch.div(DA_frame, weights) > 45, minpad, torch.div(DA_frame, weights+ EPSILON))
...
def init_constants(Fs=8000, Pow_factor=0, apply_SLL_equalization=True,
    ...
    if Fs == 16000:
        ...
        Hann_Window = torch.tensor(scipy.signal.hann(Nf, False), dtype=torch.float32).to('cuda')

        abs_thresh_power = torch.tensor(perceptual_constants.abs_thresh_power_16k, dtype=torch.float32).to('cuda')

        modified_zwicker_power = torch.tensor(perceptual_constants.modified_zwicker_power_16k, dtype=torch.float32).to('cuda')

        width_of_band_bark = torch.tensor(perceptual_constants.width_of_band_bark_16k, dtype=torch.float32).to('cuda')
        sqrt_total_width = torch.tensor(np.sqrt(np.sum(perceptual_constants.width_of_band_bark_16k)), dtype=torch.float32).to('cuda')
        # Matrices
        mat = scipy.io.loadmat(
            'H:\\JMCheng\\codes\\DCCRN_16k\\utils\\bark_matrix_16k.mat')  # Not nice with absolute path
        bark_matrix = torch.tensor(mat["Bark_matrix_16k"], dtype=torch.float32).to('cuda')
        ...
        # Matrices
        mat = scipy.io.loadmat('utils/bark_matrix_8k.mat')  # Not nice with absolute path
...
def test_PMSQE(reference_file, distorted_file):
    ...
    reference = torch.FloatTensor(reference).unsqueeze(0)
    degraded = torch.FloatTensor(degraded).unsqueeze(0)
    ref_spectra = STFT_block.transform(reference)
    deg_spectra = STFT_block.transform(degraded)
    ref_real = ref_spectra[:, :, :, 0]
    ref_imag = ref_spectra[:, :, :, 1]
    ref_mag = torch.sqrt(ref_real ** 2 + ref_imag ** 2).squeeze()  # [batch, T, F]

    deg_real = deg_spectra[:, :, :, 0]
    deg_imag = deg_spectra[:, :, :, 1]
    deg_mag = torch.sqrt(deg_real ** 2 + deg_imag ** 2).squeeze()  # [batch, T, F]

    ref_mag = ref_mag.to('cuda')
    deg_mag = deg_mag.to('cuda')
    ...
    # PMSQE computation
    PMSQE_per_frame = per_frame_PMSQE(ref_mag, deg_mag, alpha=0.1)

    # with tf.Session():
    #     pmsqe = PMSQE_per_frame.eval({signals: [reference, degraded]})


    # print("Average PMSQE distortion: ",np.mean(pmsqe))
    # plt.plot(np.arange(0, pmsqe.shape[0], 1), pmsqe)
    # plt.title('PMSQE distortion per frame')
    # plt.show()
    # pass
    return torch.mean(PMSQE_per_frame)


if __name__ == "__main__":
    # print(tf.__version__)
    # Tested on tensorflow version 1.10.0
    result = test_PMSQE('E:\JMCheng\Microsoft_dataset\DSDATASET\\test\\p226_010.wav',
                        'E:\JMCheng\Microsoft_dataset\DSDATASET\\test\\p226_010.FactoryFloor1.wav')
    print(result)
    # print("Average PMSQE distortion: ", test_PMSQE(sys.argv[1], sys.argv[2]))
